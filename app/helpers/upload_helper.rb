	...
	content_tag(:div, :id => "upload-form",  :title => "Uploading File" ) do
		form_tag('/upload/file', :multipart => true) do
			content_tag(:p) do
				...
				content_tag(:br) +
				select_tag("filetype", "<option value=\"ay-mapping\">Apply Yourself Mapping</option><option value=\"ots-mapping\">Official Test Score Mapping</option><option value=\"ay\">Apply Yourself</option><option value=\"ots\">Official Test Score</option><option value=\"pdf\">pdf file</option>".html_safe) +
				content_tag(:br) +
